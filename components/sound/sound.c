...
#define SOUND_A  26 // Audio output
#define SOUND_SD 25 // Sound disable, active low
...
int32_t sound_init(uint32_t sample_hz)
{
	...
	if (dac_handle == NULL) {
		...
		io_conf.pin_bit_mask = 1ULL<<SOUND_SD;
		io_conf.pull_down_en = 0;
		io_conf.pull_up_en = 0;
		gpio_config(&io_conf);
		gpio_set_level(SOUND_SD, device_en = true);
	}
...
// Free resources used for sound (DAC, DMA hardware, buffers, etc.).
// Return zero if successful, or non-zero otherwise.
...
// audio: a pointer to an array of 8-bit, unsigned audio data.
...
{
	gpio_set_level(SOUND_SD, device_en = enable);
}
