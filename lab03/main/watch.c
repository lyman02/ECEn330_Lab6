...
void watch_init(TFT_t *dev)
{
	...
	lcdFillScreen(dev, FACE_BG);
	lcdSetFontDirection(dev, DIRECTION0);
	lcdSetFontBackground(dev, FACE_BG);
	...
	lcdDrawString(dev, ANN_XM, ANN_Y, "MIN", ANN_C);
	lcdDrawString(dev, ANN_XS, ANN_Y, "SEC", ANN_C);

	// Set clock digit font size
	lcdSetFontSize(dev, FNT_SZ);
}
...
void watch_update(TFT_t *dev, uint32_t timer_ticks)
{
	...
	for (int32_t i = 0; i < CLK_CHR; i++) {
		if (curr_digits[i] != last_digits[i]) {
			lcdDrawChar(dev, CLK_X+i*CHR_W, CLK_Y, curr_digits[i], CLK_C);
